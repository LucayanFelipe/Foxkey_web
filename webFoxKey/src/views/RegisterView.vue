                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-6">
                      <input type="password" class="form-control form-control-user" id="exampleRepeatPassword"
                        placeholder="Repita a senha" v-model="repetirSenha">
                    </div>
                  </div>
...
export default defineComponent({
  setup() {
    ...
    const register = async () => {
      ...
      try {
        const response = await axios.post('http://localhost:5247/usuarios', {
          nome: `${nome.value} ${sobrenome.value}`,
          email: email.value,
          senha: senha.value,
          perfil_acesso: 'ADMIN' // ou outro valor permitido
        })

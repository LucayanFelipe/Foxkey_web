                    ...
                    <div class="container-fluid">
                        <div class="card shadow mb-4">
                            <div class="card-header py-3">
                                <h6 class="m-0 font-weight-bold text-primary">Consultar Estoque</h6>
                            </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table :key="dataTableKey" class="table table-bordered" id="dataTable" width="100%"
                                        ...
                                        <tbody>
                                            <tr v-for="(estoque, index) in estoques" :key="index">
                                                <td>{{ estoque.produto?.nome }}</td>
                                                <td>{{ estoque.produto?.descricao }}</td>
                                                <td>{{ estoque.produto?.codigo_barra }}</td>
                                                <td>{{ estoque.produto?.unidade_medida }}</td>
                                                <td>{{ estoque.qtd_atual }}</td>
                                                <td>{{ estoque.qtd_reservada }}</td>
                                                <td>{{ estoque.qtd_minima }}</td>
                                                <td>{{ estoque.status_estoque }}</td>
                                                <td>{{ estoque.produto?.fornecedorPf?.nome ||
                                                    estoque.produto?.fornecedorPj?.razao_social }}</td>
                                                <td>{{ estoque.observacao }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
        ...
        async fetchDados() {
            try {
                const [estoqueResponse, produtosResponse] = await Promise.all([
                    fetch('http://localhost:5247/estoques'),
                    fetch('http://localhost:5247/produtos')
                ]);

                const estoques = await estoqueResponse.json();
                const produtos = await produtosResponse.json();

                // Faz merge do produto de cada estoque com o produto completo (que tem fornecedor)
                this.estoques = estoques.map(e => {
                    const produtoCompleto = produtos.find(p => p.id_produto === e.id_produto_fk);
                    return {
                        ...e,
                        produto: {
                            ...e.produto,
                            ...produtoCompleto // inclui fornecedorPf/Pj, categoria, etc.
                        }
                    };
                });
